...
You will need to modify:
		...
		% -90dBm is noise level,  -55dBm is max RSSI (Received Signal Strength Indicator);
		% MIN_RSSI is the power reception threshold:
		% a node i accepts as neighbor each node j 
		% such that the message sent by j is received by i with
		% strength above MIN_RSSI
		% (changing MIN_RSSI does not change transmission power)
        \end{itemize}
